...
class DownloadWorker(
    ...
    private fun removeObsoleteFiles(validFiles: List<Pair<StorageReference, String>>) {
        val rootDir = applicationContext.getExternalFilesDir("pdfs") ?: return

        val validFilePaths = validFiles.map { pair ->
            File(rootDir, "${pair.second}/${pair.first.name}").absolutePath
        }.toSet()
